...
public class AvengerServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String[] urlSections = uri.split("/");// should create ["", "check", "avenger"] or
		// ["", "check", "avenger", "avenger name"]
		...
		if (urlSections.length == 3) {
			...
			int id = 0;
			try {
				id = Integer.parseInt(spacedName);
				Avenger avenger = avengerService.getSingleAvenger(id);
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (avengerService.addNewAvenger(avenger)) {
			resp.setStatus(201);
		} else {
			resp.setStatus(406);
		}
